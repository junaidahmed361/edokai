...
const QUALITY_RULES = `CRITICAL QUALITY RULES for questions: every question must have exactly 4 complete, non-truncated options with similar length (within ~3 words and similar punctuation); the correct option must NEVER be the longest or most detailed; do not make the correct answer the only option with examples/numbers; distractors must be technically plausible; vary "a" (0-3) across questions. Stems and options are complete human-readable sentences — never trailing ellipses, never cut off mid-thought. Follow The Agentic RL Frontier's pedagogy: criticals directly test a causal technical mechanism, side questions apply it in a concrete engineering scenario, and gym questions integrate concepts or expose trade-offs. Papers are evidence, not quiz subjects. NEVER put a full paper/source title in a question stem, ask what role a paper plays, where it fits, why items share a board, what a learner or Edokai should preserve, or which source/title stated a claim. A stem may use 1-2 short paragraphs or a "- " bullet list for scenario details when that aids readability.`;
